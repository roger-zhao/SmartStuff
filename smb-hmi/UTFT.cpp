...
// add by ZhaoYJ for Chinese font display
// void  ::printFont(int x, int y, int _font_x_size, int _font_y_size, uint8_t *_pFont, int _fontCnt);
void UTFT::printFont(int x, int y, int _font_x_size, int _font_y_size, uint8_t *_pFont, int _fontCnt)
{
	byte i,ch;
	word j;
	word temp; 
	...
	for(int fontIdx = 0; fontIdx < _fontCnt; fontIdx++)
	{
		//Serial.print(F("font id: "));
	...
	if (!_transparent)
	{
		if (orient==PORTRAIT)
		{
			// Serial.print(F("orient==PORTRAIT"));
			setXY(x,y,x+_font_x_size-1,y+_font_y_size-1);
	  
			temp=fontIdx*((_font_x_size/8)*_font_y_size);
			for(j=0;j<((_font_x_size/8)*_font_y_size);j++)
			{
				// from RAM, not from PGM
				//ch=pgm_read_byte(&(_pFont[temp]));
				ch = _pFont[temp];
				#if 0
				if((0 == (j%16)))
				{
		...
		else
		{
			...
			for(j=0;j<((_font_x_size/8)*_font_y_size);j+=(_font_x_size/8))
			{
				setXY(x,y+(j/(_font_x_size/8)),x+_font_x_size-1,y+(j/(_font_x_size/8)));
				for (int zz=(_font_x_size/8)-1; zz>=0; zz--)
				{
					// ch=pgm_read_byte(&(_pFont[temp+zz]));
					ch = _pFont[temp+zz];
					for(i=0;i<8;i++)
					{   
						if((ch&(1<<i))!=0)   
	...
	{
		temp=fontIdx*((_font_x_size/8)*_font_y_size);
		for(j=0;j<_font_y_size;j++) 
		{
			for (int zz=0; zz<(_font_x_size/8); zz++)
			{
				// ch=pgm_read_byte(&(_pFont[temp+zz])); 
				ch = _pFont[temp+zz];
				for(i=0;i<8;i++)
		...
		sbi(P_CS, B_CS);
		// update x axis
		x += _font_x_size; // FIXME: need to consider about TEXT rotation
}
